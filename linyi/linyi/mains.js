...
var landInformationContainerArray = new Array();//土地元素上的动画效果
landInformationContainerArray.push( about1ContainerDiv, about2ContainerDiv, about3ContainerDiv);
var seaInformationContainerArray = new Array();//海底元素上的动画效果
...
window.onresize = function (e)
{
	...
	orientLinyi();
	setLayerSpeed();
	moveLayers(); //before blinkLinyiEyes(), positionBalloonAndLinyiContainerHorizontally(), animateLinyiRunSwim(), shiftLinyiFrame(), positionContactConfirmationContainer()
	setLinyiLeftAndRightEdge();
...
function shiftDownLinyiContainer() //人物在第一个障碍物之前动画
{
	setLinyiJumpDownAndFallFrame();
	$(linyiContainerDiv).stop().animate({bottom: "8%"}, 500, function() {setLinyiStaticFrame(); enableAnimateLinyiRunSwim()});//人物跳到地面的高度
...
function setLinyiLeftAndRightEdge() //人物碰到障碍物跳起
{
...
{
		if (
		((previousPageVerticalPosition < sea1Div.offsetLeft - linyiLeftEdge) || (previousPageVerticalPosition > sea1Div.offsetLeft + sea1Div.offsetWidth - linyiRightEdge))
		&&
		((pageVerticalPosition >= sea1Div.offsetLeft - linyiLeftEdge) && (pageVerticalPosition <= sea1Div.offsetLeft + sea1Div.offsetWidth - linyiRightEdge))
		)
		{
...
function setShiftUpLayerHorizontalDistance()//跳入海底的位置
{
...
function moveUpLayerHorizontal() //人物从地面跳入海底
{
	if (layersMovement == "horizontal") 
	{
		for (var i=0; i<layerHorizontalArray.length; i++)
...
function shiftLinyiToGroundLevel()//从海里到地面的高度
{
	$(linyiContainerDiv).stop().animate({bottom: containerDiv.offsetHeight - groundAndGrassContainer1Div.offsetTop-10 + "px"}, 300, function() {});
}

function shiftLinyiToSeaFloor()//到海底过程的高度
{
	$(linyiContainerDiv).stop().animate({bottom: seaFloorDiv.offsetHeight + "px"}, 300, function() {});
}
...
function linyiJumpUp(i)//前进后退碰到障碍物跳起代码
{
		
		if (((previousPageVerticalPosition <= elevationArray[i].offsetLeft - linyiRightEdge) && (pageVerticalPosition > elevationArray[i].offsetLeft - linyiRightEdge))
		||
		
		((previousPageVerticalPosition >= elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - linyiLeftEdge) && (pageVerticalPosition < elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - linyiLeftEdge)))
		{
			positionLinyiAtGroundLevel(); 
			$(linyiContainerDiv).stop().animate({bottom: [containerDiv.offsetHeight - groundAndGrassContainer1Div.offsetTop + 200, 'easeOutCubic']}, 300, function() {linyiJumpDown(i)});
			setLinyiJumpUpFrame();
		}	
}

function linyiJumpDown(i)//前进后退碰到障碍物跳起后着落在障碍物代码
{
		...
		{
			$(linyiContainerDiv).stop().animate({bottom: [containerDiv.offsetHeight - elevationArray[i].offsetTop-10, 'easeInCubic']}, 300, function() {disableIsLinyiJumpingAndFalling(); setLinyiStaticFrame()});
			setLinyiJumpDownAndFallFrame();
		}
}

function linyiFall(i)//前进后退从障碍物跳下代码
{
		
		if (((previousPageVerticalPosition < elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - linyiLeftEdge) && (pageVerticalPosition >= elevationArray[i].offsetLeft + elevationArray[i].offsetWidth - linyiLeftEdge))
		||
		
		((previousPageVerticalPosition > elevationArray[i].offsetLeft - linyiRightEdge) && (pageVerticalPosition <= elevationArray[i].offsetLeft - linyiRightEdge)))
		{
			isLinyiFalling = true;
			setLinyiJumpDownAndFallFrame();
			$(linyiContainerDiv).stop().animate({bottom: [containerDiv.offsetHeight - groundAndGrassContainer1Div.offsetTop-10, 'easeInCubic']}, 300, function() {disableIsLinyiJumpingAndFalling(); setLinyiStaticFrame()});
		}
}
...
function linyiSwimUp()
{
	...
	if (swimUpHeight > 0) 
	{
	 var swimUpDistance = seaFloorDiv.offsetHeight + swimUpHeight + "px";
		...
		$(linyiContainerDiv).stop().animate({bottom: swimUpDistance}, linyiSwimUpTime, function() {linyiSwimDown(linyiSwimDownTime)});
	}
}

function linyiSwimDown(linyiSwimDownTime)
{
	$(linyiContainerDiv).stop().animate({bottom: seaFloorDiv.offsetHeight + "px"}, linyiSwimDownTime, function() {setLinyiStaticFrameonsea()});
...
function animateLinyiEyes()
{
	...
	blinkLinyiEyesTimer = setInterval(function(){blinkLinyiEyes()}, 4000);
}
...
function blinkLinyiEyes()
{
	...
	{
		$(sweatDiv).fadeTo(0,1);
		$(sweatDiv).stop().delay(300).animate({opacity: 0}, 0, function() {});
	}
}
...
{
	swimUpHeight = Math.abs(deltaPageVerticalPosition);//Math.abs()取绝对值函数
...
function setLayersMovement()
{
	...
	else if ((pageVerticalPosition >= pageDiv.offsetHeight - containerDiv.offsetHeight - distanceBetweenlinyiAndhuojian) && (pageVerticalPosition < pageDiv.offsetHeight - containerDiv.offsetHeight))
	{
...
function animatewBuildings()
{
	...
	{
		$(buildingwArray[i]).stop().delay(i * 300).animate({left: [buildingwTargetLeftArray[i], 'easeOutCubic']}, 1200, function() {});
	}
}
...
function animateLinyiRunSwim()
{
	if ((canAnimateLinyiRunSwim == true) && (isLinyiJumping == false) && (isLinyiFalling == false) && (layersMovement != "vertical"))
	{
		...
		shiftLinyiFrameTimer = setInterval(function(){shiftLinyiFrame()}, shiftLinyiFrameTimeInterval);
	}
}
...
function shiftLinyiFrame()
{
	...
	else if ((isLinyiSwimming == true) && (isLinyiBelowSeaLevel == true))
	{
...
function clearShiftLinyiFrameTimer()
{
	...
	if
	((isLinyiSwimming == true) && (linyiContainerDiv.offsetTop + linyiContainerDiv.offsetHeight >= containerDiv.offsetHeight - seaFloorDiv.offsetHeight)) 
	{
		setLinyiStaticFrameonsea();
	}
	counter = 0;
...
function animateInformationAndEnemiesElements()
{
	if (layersMovement == "horizontal")
	{
		if (isLinyiSwimming == false)
		{
			...
			{
				if (((previousPageVerticalPosition + (0.5 * containerDiv.offsetWidth) > landInformationContainerArray[i].offsetLeft) || (previousPageVerticalPosition + (0.5 * containerDiv.offsetWidth) > landInformationContainerArray[i].offsetLeft + 1000))
			&& 
			((pageVerticalPosition + (0.5 * containerDiv.offsetWidth) > landInformationContainerArray[i].offsetLeft) && (pageVerticalPosition + (0.5 * containerDiv.offsetWidth) >landInformationContainerArray[i].offsetLeft + 1000)))
				{
					//动车
...
function animateBubble()
{
	...
	$(bubbleDiv).stop().animate({top: "0px"}, 2 * linyiToSeaSurfaceDistance, function() {hideBubble()});
}
...
function happyLinyi()
{
	...
	{	
		...
		happyLinyiTimer = setInterval(function(){linyiHandsUp()}, 3000); 
		isLinyiHappy = true;
	}
}
...
function linyiHandsUp()
{
	...
	setTimeout(function() {setLinyiStaticFrame()}, 1000); 
}
...
function resetFireworkSvg()
{
 ...
 var bird_small_dir = 0;
    (function birdSmallAction(){
        $(".bird_small").animate({"top": 200+(80*bird_small_dir)}, 700, function(){
...
$(".bike_left").animate({"right": 100+(800*bike_left_dir)}, 600, function(){
...
$(".lovers_right").animate({"right": 10+(800*lovers_right_dir)}, 600, function(){
...
$(".teacher_left").animate({"right": 10+(800*lovers_right_dir)}, 600, function(){
...
function animatePlants()
{
	...
	{
		$(plantArray[i]).stop().delay(i * 300).animate({top: [plantTargetTopObjectArray[i].offsetTop, 'easeOutElastic']}, 3000, function() {});
	}
}
