                ...
                <div class="cart-group">
                  ...
                  <div class="cart-top-items">
                    <div class="cart-items" v-for="item,index in carPanelData" :key="index">
                      ...
                      <div class="operation">
                        <a class="items-delete-btn" @click="deleteCarpanelData(item.sku_id)"></a>
                      </div>
                      <div class="subtotal">¥ {{item.price*item.count}}</div> <!-- 小计价格* 数量X单价 -->
                      ...
                      <div class="price">¥ {{item.price}}</div>  <!-- 单价 -->
                    </div>
                  </div>
            ...
            <div class="cart-bar-operation">
              <div>
                <div class="choose-all js-choose-all">
                  <span class="blue-checkbox-new " :class="{'checkbox-on':allCheckedHandle}" @click="allCheckGoods(allCheckedHandle)"><a></a></span>
                  全选
                </div>
                <div class="delete-choose-goods" @click="deleteCheckedData">删除选中的商品</div>
              </div>
            </div>
            <div class="shipping">
              <div class="shipping-box">
                <div class="shipping-total shipping-num">
                  ...
                  <h5>
                    共计 <i >{{totalCount}}</i> 件商品  <!-- 总计购物车商品数量 -->
                  </h5>
                </div>
              ...
              <router-link to='/checkout' tag="span" class="jianguo-blue-main-btn big-main-btn js-checkout " 
              :class="{ 'disabled-btn':checkedCount<1 }">
...
export default{
    computed: {
      carPanelData (){ // 返回store里面的购物车数据
        return this.$store.state.carPanelData
      },
      totalCount (){ //返回总计购物车商品数量
        return this.$store.getters.totleCount
      },
      totalPrice (){ //返回总计购物车商品价格
        return this.$store.getters.totlePrice
      },
      checkedCount(){  //返回购物车商品选中的数量
        return this.$store.getters.checkedCount
      },
      checkedPrice(){  //返回购物车选中的商品的总价
        return this.$store.getters.checkedPrice
      },
      allCheckedHandle(){   //返回全选按钮的状态
        return this.$store.getters.allCheckedHandle
      }
    },
    methods: {
      deleteCarpanelData(id){ //删除购物车清单的商品
        this.$store.commit('deleteCarpanelData',id)
      },
      checkGoods(id){ //选中购物车清单的商品
        this.$store.commit('checkGoods',id)
      },
      plusCarpanelhandle(id){ //点击+号增加购物车商品数量
        this.$store.commit('plusCarpanelhandle',id)
      },
      subCarpanelhandle(id){ //点击-号减少购物车商品数量
        this.$store.commit('subCarpanelhandle',id)
      },
      allCheckGoods(checked){   //点击全选按钮切换状态
        this.$store.commit('allChecked',checked)
      },
      deleteCheckedData(){      //删除选中(checked)的购物车商品
        this.$store.commit('deleteCheckedData')
      }
    }
}
</script>
